import { Component, NgZone, ElementRef, Renderer2, ViewChild, OnDestroy, ChangeDetectorRef  } from '@angular/core';
...
export class TestComponent implements OnDestroy {
  ...
  constructor(private api: SerrviceService, private ngZone: NgZone, private datePipe: DatePipe, private cdr: ChangeDetectorRef,
  ...
  dataa = {session_id: this.sessionId,user_name:this.api.userName,user_message: this.userM, edit_field: "", confirmation : "False"};
...
// textarea.addEventListener('input', this.toggleeButton);
  ...
  onFileSelected(event: any) {
    ...
    this.fileExtension = fileName.substring(fileName.lastIndexOf('.')).toLowerCase();

    // Check file type
    if (!allowedExtensions.includes(this.fileExtension)) {
      this.errorDivText = "Files of following format is not supported" + " "+this.fileExtension;
  ...
  onFileAttach(event: any){
    ...
    if (file.size > maxSize) {
     ...
     if (this.addfileExtension === '.doc' || this.addfileExtension === '.docx') {
       this.fileIcon = 'assets/images/docs.png'; // Replace with actual icon path
     } else if (this.addfileExtension === '.ppt' || this.addfileExtension === '.pptx') {
       this.fileIcon = "assets/images/ppt1.png"; // Replace with actual icon path
     } else if (this.addfileExtension === '.pdf') {
       this.fileIcon = "assets/images/download.png"; // Replace with actual icon path
     } else if (this.addfileExtension === '.xls' || this.addfileExtension === '.xlsx') {
      this.fileIcon = "assets/images/xl.png"; // Replace with actual icon path
     } else {
       this.fileIcon = "assets/images/download(1)2.png"; // Replace with a default icon
     }
            ...
            if(this.botButtonResponse !== null){
              if(this.botButtonResponse.length > 0 && this.apiResponseData.drop_down == true){
                ...
                if(this.checkIfArray(this.botChatMessage['Guidelines'])){
                  ...
                  this.chatHistory.push({ text: {question: this.botChatMessage['Question'],guidelines: this.splitByDot(this.botChatMessage['Guidelines'])}, button :this.botButtonResponse,  sender: 'bot'});
                }
  ...
  additionalDataForSubmit(){
    ...
    let additionalData = { 
      ...
      update_data : {
        ...
        Submitted_date: this.datePipe.transform(new Date(), 'dd-MM-yyyy HH:mm:ss'),
